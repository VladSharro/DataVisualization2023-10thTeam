...
public class VisViolin : Vis
{
    ...
    public override GameObject CreateVis(GameObject container)
    {
        ...
        double max = (dataSets[0].ElementAt(1).Value).Max();
        double min = (dataSets[0].ElementAt(1).Value).Min();
        //double[] sorted = (dataSets[0].ElementAt(1).Value).Sort();
        //int calc = sorted.Length / 2;
        //double median = sorted[calc];
